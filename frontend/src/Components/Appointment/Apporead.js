...
const Apporead = () => {
  ...
  const handleDeleteAppo = (apptid) => {
    axios.delete(`http://localhost:3000/api/apptmt/${apptid}`)
      .then(response => {
  ...
  const handleFetchData = () => {
    axios.get(`http://localhost:3000/api/apptmt/${fetchId}`)
      .then(response => {
    ...
    <div className='card-body'>
      ...
      {fetchedAppo ? (
        <div className="ps">
          ...
          <p style={{ color: 'green' }}>Appointment id: {fetchedAppo[0]}</p>
          <p style={{ color: 'green' }}>Patient ID: {fetchedAppo[1]}</p>
          <p style={{ color: 'green' }}>Staff ID: {fetchedAppo[2]}</p>
          <p style={{ color: 'green' }}>Appointment Date & Time: {fetchedAppo[3]}</p>
          <p style={{ color: 'green' }}>Appointment Type: {fetchedAppo[4]}</p>
          <p style={{ color: 'green' }}>Status: {fetchedAppo[5]}</p>
